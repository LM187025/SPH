...
export default {
    ...
    methods:{
        ...
        goSearch(){
            ...
            // 第二种：模板字符串
            // this.$router.push(`/search/${this.keyword}?k=${this.keyword.toUpperCase()}`);
            // 第三种：对象  参数有params时 跳转路径属性不能使用path
            ...
            // 如何指定params参数可以传递，或者是不传递，在配置路由的时候，在占位符的后面加上一个问号【代表params可传递可不传递】
            ...
            // 代表的是如果有query参数也带过去
            if(this.$route.query){
                let loction = {name:'search',params:{keyword:this.keyword || undefinded}}
            }
