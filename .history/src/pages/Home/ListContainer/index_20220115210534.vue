...
export default {
  ...
  mounted() {
    ...
    // mounted：组件挂载完毕，正常说组件结构（DOM）已经完全有了
    // 为什么swiper实例在mounted当中不能直接书写：因为结构还没有完整（涉及到异步ajax数据请求）
    setTimeout(() => {
  ...
  watch: {
    //   监听bannerList数据的变化：因为这条数据发生变化---由空数组变为数组里面有四个元素
    bannerList: {
      handler(newValue, oldValue) {
        ...
        // nextTick：下次DOM更新，循环结束之后，执行延迟回调，在修改数据之后立即使用这个方法，获取更新后的
        var mySwiper = new Swiper(document.querySelector('.swiper-container'), {
          loop: true,
          // 如果需要分页器
          pagination: {
